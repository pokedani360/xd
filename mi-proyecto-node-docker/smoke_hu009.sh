...
get_token() {
  ...
  [ "$c" -lt 300 ] || { say "❌ Login $email ($c): $b"; exit 1; }
  local t="$(printf "%s" "$b" | jq -r '.token // empty')"
  [ -n "$t" ] || { say "❌ Sin token para $email: $b"; exit 1; }
  echo "$t"
}
...
[ "$(code "$resp")" -lt 300 ] || { say "❌ Ensayo perm ($(code "$resp")): $(body "$resp")"; exit 1; }
...
say "\n--- Paso 7: Rendiciones ---"
curl -sS -i -X POST "$BASE_URL/api/resultados/rendiciones" -H "Authorization: Bearer $TOKEN_ALUMNO" -H "Content-Type: application/json" -d "$(jq -n --argjson ventana_id "$ID_VENTANA_ACTIVA" '{ventana_id:$ventana_id}')"
curl -sS -i -X POST "$BASE_URL/api/resultados/rendiciones" -H "Authorization: Bearer $TOKEN_ALUMNO" -H "Content-Type: application/json" -d "$(jq -n --argjson ventana_id "$ID_VENTANA_ACTIVA" '{ventana_id:$ventana_id}')"
curl -sS -i -X POST "$BASE_URL/api/resultados/rendiciones" -H "Authorization: Bearer $TOKEN_ALUMNO" -H "Content-Type: application/json" -d "$(jq -n --argjson ensayo_id "$ID_ENSAYO_PERM" '{ensayo_id:$ensayo_id}')"
curl -sS -i -X POST "$BASE_URL/api/resultados/rendiciones" -H "Authorization: Bearer $TOKEN_ALUMNO" -H "Content-Type: application/json" -d "$(jq -n --argjson ensayo_id "$ID_ENSAYO_PERM" '{ensayo_id:$ensayo_id}')"

say "\n--- Paso 8: Ventana FUTURA + verificación 403 ---"
MANANA="$(date -d '+1 day' --iso-8601=seconds --utc)"
resp="$(post_json "$BASE_URL/api/ensayos/${ID_ENSAYO_VENTANA}/ventanas" "$TOKEN_DOCENTE" "$(jq -n --argjson curso_id "$ID_CURSO" --arg inicio "$MANANA" --argjson duracion_min 60 '{curso_id:$curso_id, inicio:$inicio, duracion_min:$duracion_min}')")"
[ "$(code "$resp")" -lt 300 ] || { say "❌ Ventana FUTURA ($(code "$resp")): $(body "$resp")"; exit 1; }
ID_VENTANA_FUTURA="$(printf "%s" "$(body "$resp")" | jq -r '.id')"
curl -sS -i -X POST "$BASE_URL/api/resultados/rendiciones" -H "Authorization: Bearer $TOKEN_ALUMNO" -H "Content-Type: application/json" -d "$(jq -n --argjson ventana_id "$ID_VENTANA_FUTURA" '{ventana_id:$ventana_id}')"

say "\n✅ Smoke HU-009 finalizado correctamente"